...
! execute_startup_init "$SCRIPT_DIR" "$JOSP_DIR" "localhost" 8998 9001 9004 && echo "Error initializing startup script" && exit
...
echo "* Startup JCP APIs as gradle task..."
! execute_startup_gradle_task_daemon "jcpAPIs_Start" "$TEST_LOG_DIR/execute_startup_gradle_task_daemon-jcpAPIs_Start.log" \
  && echo "Error on startup JCP APIs, exit" && exit
echo "* Startup JCP GWs as gradle task..."
! execute_startup_gradle_task_daemon "jcpGWs_Start" "$TEST_LOG_DIR/execute_startup_gradle_task_daemon-jcpGWs_Start.log" \
  && echo "Error on startup JCP GWs, exit" && exit
echo "* Startup JCP JSLWebBridge as gradle task..."
! execute_startup_gradle_task_daemon "jcpJSLWebBridge_Start" "$TEST_LOG_DIR/execute_startup_gradle_task_daemon-jcpJSLWebBridge_Start.log" \
  && echo "Error on startup JCP JSLWebBridge, exit" && exit
echo "* Startup JCP FrontEnd as gradle task..."
! execute_startup_gradle_task_daemon "jcpFE_Start" "$TEST_LOG_DIR/execute_startup_gradle_task_daemon-jcpFE_Start.log" \
  && echo "Error on startup JCP FrontEnd, exit" && exit

execute_startup_print_end "$TEST_LOG_DIR" "$SCRIPT_DIR" "$JCP_HOST" "$JCP_AUTH_PORT" "$JCP_APIS_PORT" "$JCP_FE_PORT"
exit 0
