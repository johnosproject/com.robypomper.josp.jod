...
public abstract class AbsJODState extends AbsJODComponent
    ...
    private static final Logger log = LoggerFactory.getLogger(AbsJODState.class);
    ...
    protected AbsJODState(JODStructure structure, JODExecutorMngr execMngr, JODHistory history, String name, String descr, String listener, String puller) throws JODStructure.ComponentInitException {
        ...
        if (listener == null && puller == null) {
            log.warn(String.format("Error on setting state component '%s' listener or puller because no listener nor puller given", getName()));
        ...
        if (listener != null) {
            if (listener.equalsIgnoreCase("NONE")) {
                log.info(String.format("State component '%s' configured without a listener worker", getName()));

            } else {
                try {
                    log.trace(String.format("Setting state component '%s' listener '%s'", getName(), listener));
                    ...
                    log.warn(String.format("Error on setting state component '%s' listener because %s", getName(), e.getMessage()), e);
        ...
        if (puller != null) {
            if (!puller.equalsIgnoreCase("NONE")) {
                log.info(String.format("State component '%s' configured without a puller worker", getName()));

            } else {
                try {
                    log.trace(String.format("Setting state component '%s' puller '%s'", getName(), puller));
    ...
    public void propagateState(JODStateUpdate update) throws JODStructure.CommunicationSetException {
        log.debug(String.format("Propagating component '%s' state", getName()));
        ...
        log.debug(String.format("Component '%s' propagated state '%s'", getName(), update.encode()));
    }
